...
## Description
**ConfigAssist** will automatically generate a web form with variable controls, based on a **text description** string, allowing editing of application variables such as **Wifi ssid**, **Wifi password**, **host_name**. A json description text, with a `variable name`, a `default value` a `label` and `attributes` for each variable must be defined describing the variable type of all the variables that the application will use.

A simple html page will automatically generated by **ConfigAssist** allowing editing of these variables from a web 
Browser with connection to the device. After editing and saving, application variables can be used with operator **[]** i.e. ```conf['variable']```.
